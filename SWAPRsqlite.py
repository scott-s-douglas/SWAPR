    ...
    def finalize(self, labNumber, seed, N, MOOC=False):
        ...
        if len(pseudoURL.keys()) > 0:
            ...
            for key in pseudoURL.keys():
                ...
                for i in range(0, len(URLSToGrade)):
                    ...
                    self.cursor.execute("INSERT INTO assignments VALUES(?, ?, ?, ?)", [labNumber, key, i+1, URLSToGrade[i]])
                self.conn.commit()
        ...
        if len(data) > N:
            for d in data:
                ...
                for i in range(0, len(URLSToGrade)):
                    #i+1 because we want item index to start at 1
                    self.cursor.execute("INSERT INTO assignments VALUES(?, ?, ?, ?)", [labNumber, URLTowIDDict[d], i+1, URLSToGrade[i]])
        self.conn.commit()
        
        ##checks to see if each URL was assigned 3 or 4 times
        #self.cursor.execute("SELECT URLToGrade from assignments")
        #checks = [str(d[0]) for d in self.cursor.fetchall()]
        
        #for URL in set(checks):
        #    print checks.count(URL), URL
